...
export default function WhatYouGain({ data }: WhatYouGainProps) {
  return (
    <section className="py-4 md:py-12 bg-black text-white px-4 md:px-12">
          ...
          {data.cards.map((card, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Content */}
                <div className="absolute inset-0 p-3 pb-3 flex flex-col justify-end">
                  <div className="mb-3 text-white/90 group-hover:text-white transition-colors">
                    {Icon && <Icon className="w-8 h-8 md:w-8 md:h-8" />}
                  </div>
                  <p className="text-white text-md md:text-xl leading-[1.3] transition-all group-hover:text-white">
                    {card.description}
                  </p>
                </div>
              </motion.div>
